                ...
                <div className="lg:col-span-8 bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden">
                    <div className="p-8 lg:p-10 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                            <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest px-3 py-1 bg-emerald-50 border border-emerald-100 rounded-full">Filter: Unresolved</span>
                        </div>
                    </div>
                        ...
                        {riskAlerts.map((alert) => (
                            <div key={alert.id} className="p-8 flex items-center justify-between hover:bg-slate-50 transition-all group">
                                <div className="flex items-center gap-6">
                                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${alert.impact === 'CRITICAL' ? 'bg-rose-500/10 text-rose-500' : alert.impact === 'HIGH' ? 'bg-amber-500/10 text-amber-500' : 'bg-blue-500/10 text-blue-500'}`}>
                                        <FaExclamationTriangle className="w-6 h-6" />
                                    </div>
                                    <div>
                                        <div className="flex items-center gap-3 mb-1">
                                            <h4 className="text-base font-black text-[#1E293B]">{alert.type}</h4>
                                            <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${alert.impact === 'CRITICAL' ? 'bg-rose-500 text-white' : 'bg-slate-100 text-slate-500'}`}>{alert.impact}</span>
                                        </div>
                                ...
                                <div className="flex items-center gap-8">
                                    ...
                                    <button className="px-6 py-3 bg-white border border-slate-200 text-[#1E293B] text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#10367D] hover:text-white hover:border-[#10367D] transition-all">
                                        Investigate
                                    </button>
                                </div>
                            </div>
                            ...
                            {[1, 2].map((i) => (
                                <div key={i} className="p-5 bg-white/5 rounded-2xl border border-white/10 hover:bg-white/10 transition-all">
                                    <div className="flex items-center justify-between mb-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-xl bg-[#10367D]/20 flex items-center justify-center text-[#10367D]">
                    ...
                    <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100">
                        ...
                        <div className="space-y-3">
                            {['Order Velocity Limit', 'Refund Threshold', 'Login Geo-Guard', 'Document MD5 Hash Check'].map((preset) => (
                                <div key={preset} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100 group cursor-pointer hover:border-[#10367D]/20 transition-all">
                                    <span className="text-[10px] font-black text-[#1E293B] uppercase tracking-widest">{preset}</span>
                                    <div className="w-8 h-4 bg-emerald-500 rounded-full p-1 flex justify-end">
