...
export const getCurrentUser = async (req, res) => {
    try {
        ...
        res.json({
            ...
            role: user.role,
            status: user.status,
            avatar: user.avatar // Ensure schema has this or remove if not
        });
    } catch (error) {
        res.status(500).json({ error: 'SESSION_HYDRATION_FAILED' });
    }
};

export default { register, login, loginWithPhone, googleLogin, getCurrentUser };
