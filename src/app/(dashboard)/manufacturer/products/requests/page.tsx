    ...
    return (
        <div className="space-y-6 animate-fade-in pb-12">
            <div className="flex flex-col gap-2">
                <h1 className="text-3xl font-black tracking-tight text-slate-900">Product Allocations</h1>
                <p className="text-slate-400 font-bold uppercase tracking-widest text-sm">Manage stock requests from your retail partners.</p>
            </div>

            {requests.length === 0 ? (
                <div className="bg-white rounded-[10px] p-12 text-center border border-slate-100 border-dashed">
                    ...
                    <AnimatePresence>
                        {requests.map((request) => (
                            <motion.div
                                ...
                                className="bg-white rounded-[10px] p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all group"
                            >
                                <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
                                    {/* Product Image */}
                                    <div className="w-16 h-16 bg-slate-100 rounded-[10px] overflow-hidden flex-shrink-0 border border-slate-200">
                                        {request.productId?.images?.[0] ? (
                                            <img src={request.productId.images[0]} alt="" className="w-full h-full object-contain" />
                                        ) : (
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <span className="px-2 py-0.5 bg-amber-50 text-amber-700 text-sm font-bold uppercase tracking-wider rounded-full border border-amber-100 flex items-center gap-1">
                                    ...
                                    <div className="flex items-center gap-3 w-full md:w-auto">
                                        ...
                                        <button
                                            ...
                                            disabled={!!processingId}
                                            className="flex-1 md:flex-none px-6 py-2 bg-[#067FF9] text-white text-xs font-bold uppercase tracking-wider rounded-[10px] hover:bg-[#0d5a9e] transition-colors shadow-lg shadow-blue-500/20 disabled:opacity-50 flex items-center justify-center gap-2"
                                        >
