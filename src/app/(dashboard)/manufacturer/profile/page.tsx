    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 gap-6">
                ...
                {/* Document List would go here - currently relying on KYC Modal */}
                <div className="text-center p-8 bg-slate-50 rounded border border-dashed border-slate-200">
                    <p className="text-sm text-slate-500 mb-4">Manage compliance documents and GST verification via the Verification settings.</p>
                    <button
                        onClick={() => document.getElementById('kyc-trigger')?.click()}
                        className="px-6 py-2 bg-slate-200 text-slate-700 text-xs font-bold tracking-wide rounded-[6px] hover:bg-slate-300 transition-all"
                    >
                        Manage Documents
                    </button>
                </div>
...
function BankSection({ profile, onSave, isSaving }: any) {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2 space-y-1.5">
                    <label className="text-xs font-semibold text-slate-500 tracking-wide ml-1">Bank Name</label>
                    <input className="w-full bg-slate-50 border border-slate-200 rounded-[10px] py-3 px-4 text-sm font-medium focus:outline-none focus:border-slate-400" value={formData.bankName} onChange={e => setFormData({ ...formData, bankName: e.target.value })} />
                </div>
                <div className="md:col-span-2 space-y-1.5">
                    <label className="text-xs font-semibold text-slate-500 tracking-wide ml-1">Account Holder Name</label>
                    <input className="w-full bg-slate-50 border border-slate-200 rounded-[10px] py-3 px-4 text-sm font-medium focus:outline-none focus:border-slate-400" value={formData.accountName} onChange={e => setFormData({ ...formData, accountName: e.target.value })} />
                </div>
                <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-500 tracking-wide ml-1">Account Number</label>
                    <input className="w-full bg-slate-50 border border-slate-200 rounded-[10px] py-3 px-4 text-sm font-medium focus:outline-none focus:border-slate-400" value={formData.accountNumber} onChange={e => setFormData({ ...formData, accountNumber: e.target.value })} />
                </div>
                <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-500 tracking-wide ml-1">IFSC Code</label>
                    <input className="w-full bg-slate-50 border border-slate-200 rounded-[10px] py-3 px-4 text-sm font-medium focus:outline-none focus:border-slate-400 uppercase" value={formData.ifscCode} onChange={e => setFormData({ ...formData, ifscCode: e.target.value })} />
                </div>
                <div className="md:col-span-2 space-y-1.5">
                    <label className="text-xs font-semibold text-slate-500 tracking-wide ml-1">Branch Name</label>
                    <input className="w-full bg-slate-50 border border-slate-200 rounded-[10px] py-3 px-4 text-sm font-medium focus:outline-none focus:border-slate-400" value={formData.branchName} onChange={e => setFormData({ ...formData, branchName: e.target.value })} />
                </div>
            </div>

            <div className="p-4 bg-amber-50 border border-amber-100 rounded-[8px] flex items-start gap-3">
                <FaExclamationTriangle className="text-amber-500 w-4 h-4 mt-0.5" />
                <div>
                    <p className="text-xs font-bold text-amber-800">Security Notice</p>
                    <p className="text-xs text-amber-600 mt-1">Updating bank details will trigger a mandatory re-verification process. Your account status will be set to PENDING until approved.</p>
                </div>
            </div>

            <button onClick={() => onSave({ bankDetails: formData })} disabled={isSaving} className="px-6 py-3 bg-slate-900 text-white text-xs font-bold tracking-wide rounded-[10px] shadow-lg shadow-slate-200 hover:bg-slate-800 transition-all flex items-center gap-2">
                <FaSave className="w-3.5 h-3.5" /> {isSaving ? 'Directing...' : 'Update Fiscal Data'}
            </button>
        </div>
    );
}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {profile.dealersApproved?.length > 0 ? (
                    ...
                    <div className="md:col-span-2 p-12 bg-slate-50 border border-dashed border-slate-200 rounded-[10px] text-center space-y-4">
                        ...
                        <Link href="/manufacturer/sellers" className="text-xs font-bold text-slate-900 tracking-wide hover:underline inline-block mt-4">Manage Partner Requests →</Link>
                    </div>
                )}
            </div>
        </div>
            ...
            <div className="space-y-6">
                ...
                <div className="p-6 bg-slate-50 border border-slate-200 rounded-[10px] flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm font-medium text-slate-400 mt-0.5 text-rose-500">Contact Admin to deactivate account</p>
                    </div>
                    <button disabled className="px-6 py-2.5 bg-slate-100 text-slate-400 text-xs font-bold tracking-wide rounded-[8px] cursor-not-allowed">Deactivate</button>
                </div>
            </div>
        </div>
    );
