...
export default function SellerInventoryPage() {
  ...
  const fetchInventory = async () => {
    setIsLoading(true);
    try {
      const data = await apiClient.get<any[]>("/seller/inventory");
      setInventory(data);
    } catch (error) {
  ...
  const handleUpdateStock = async (id: string, newStock: string) => {
    setIsUpdating(id);
    try {
      await apiClient.put("/seller/inventory/stock", {
        inventoryId: id,
  ...
  return (
    <div className="space-y-6 pb-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-[#1E293B] tracking-tight italic">
            Inventory <span className="text-[#10367D]">Control</span>
          </h1>
          <p className="text-sm font-bold text-slate-400 mt-1 uppercase tracking-widest text-[10px]">
            Real-time regional fulfillment spine
          </p>
        </div>
        <div className="flex gap-3">
          ...
          >
            <FaSync
              className={`w-3 h-3 text-slate-400 ${isLoading ? "animate-spin" : ""}`}
            />
            Refresh
          </button>
        </div>
                ...
                inventory.map((item) => (
                  ...
                  >
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">
                              ALLOCATED: {item.allocatedStock || 0} UNITS
                            </p>
                          </div>
                        </div>
                      </div>
                    ...
                    <td className="px-8 py-6 text-xs font-bold text-slate-400 italic">
                      ₹{item.dealerBasePrice || item.product.basePrice}
                    </td>
                    <td className="px-8 py-6">
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div
                            className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ${item.stock > 0 ? "bg-blue-50 text-[#10367D]" : "bg-rose-50 text-rose-600"}`}
                          >
                            {item.stock} Units
                          </div>
                        </div>
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-2">
                        <div
                          className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest w-fit ${item.isListed ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : "bg-slate-50 text-slate-400 border border-slate-100"}`}
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${item.isListed ? "bg-emerald-500 animate-pulse" : "bg-slate-300"}`}
                          ></span>
                    ...
                    <td className="px-8 py-6 text-right">
                      <div className="flex flex-col items-end gap-2">
                        <Link
                          href={`/seller/inventory/list/${item.id}`}
