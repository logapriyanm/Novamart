...
export default function SourcedProductsPage() {
    ...
    const [selAlloc, setSelAlloc] = useState<any>(null);
    ...
    const fetchAllocations = async () => {
        ...
        try {
            ...
            toast.error('Failed to load sourced products');
        } finally {
            setIsLoading(false);
        }
    };

    const handleSourceAndList = async (e: React.FormEvent) => {
        e.preventDefault();
        setIsSubmitting(true);
        try {
            const res = await apiClient.post('/seller/source', {
                productId: selAlloc.productId._id || selAlloc.productId, // Ensure ID is used
    ...
    // Filter only listed/sourced items if the page is strictly "Sourced Products"
    // Or show all allocations if that's the intent.
    // The sidebar says "Sourced Products", typically implies things I have sourced.
    // But "Allocations" implies things available to source.
    // Given the UI shows both ("Available Slots" vs "Active Listing"), I will keep both.
    ...
    return (
        <div className="space-y-8 animate-fade-in pb-12 text-[#1E293B]">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black tracking-tight italic">Sourced <span className="text-[#10367D]">Products</span></h1>
                    <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-1">Manage Manufacturer Allocations & Retail Listings</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {isLoading ? (
                    Array(3).fill(0).map((_, i) => (
                        <div key={i} className="h-80 bg-slate-100 rounded-[10px] animate-pulse"></div>
                    ))
                ) : allocations.length === 0 ? (
                    <div className="col-span-full py-20 text-center">
                        <MdOutlineProductionQuantityLimits className="w-16 h-16 text-slate-200 mx-auto mb-6" />
                        <h3 className="text-xl font-black italic text-slate-400 uppercase tracking-widest">No sourced products found</h3>
                    ...
                    <motion.div
                        ...
                        whileHover={{ y: -8 }}
                        className={`bg-white rounded-[10px] border ${item.isListed ? 'border-emerald-100 shadow-emerald-100/50' : 'border-slate-100'} shadow-xl overflow-hidden group relative transition-all`}
                    >
                        {item.isListed && (
                        ...
                        <div className="aspect-[4/3] bg-slate-50 relative overflow-hidden">
                            {item.productId?.images?.[0] ? (
                                <img src={item.productId.images[0]} alt={item.productId.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                            ) : (
                            ...
                            <div className="absolute bottom-6 left-6 right-6 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                                ...
                                <h4 className="text-white text-lg font-black italic truncate">{item.productId?.manufacturerId?.companyName || 'Unknown Manufacturer'}</h4>
                            </div>
                        </div>

                        <div className="p-8 space-y-6">
                            <div>
                                <h4 className="text-xl font-black italic text-[#1E293B] group-hover:text-[#10367D] transition-colors">{item.productId?.name}</h4>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="p-4 bg-blue-50/50 rounded-[10px] border border-blue-100/50">
                                    ...
                                    <p className="text-lg font-black text-blue-600 italic">₹{Number(item.sellerBasePrice || item.productId?.basePrice).toLocaleString()}</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center justify-between text-[10px] font-bold">
                                    ...
                                    <span className="text-amber-600 font-black">{item.sellerMoq || item.productId?.moq || 1} Units</span>
                                </div>
                                <div className="flex items-center justify-between text-[10px] font-bold">
                            ...
                            {item.isListed ? (
                                <button className="w-full py-4 bg-white border-2 border-slate-100 text-slate-400 rounded-[10px] font-black text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-3">
                                ...
                                <button
                                    onClick={() => {
                                        setSelAlloc(item);
                                        setFormData({ ...formData, retailPrice: Number(item.sellerBasePrice || item.productId?.basePrice).toString() });
                                        setShowListingModal(true);
                                    }}
                                    className="w-full py-4 bg-[#10367D] text-white rounded-[10px] font-black text-[10px] uppercase tracking-[0.2em] shadow-xl shadow-[#10367D]/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3"
                                >
                            ...
                            <form onSubmit={handleSourceAndList} className="space-y-8">
                                <div className="p-6 bg-blue-50 border border-blue-100 rounded-[2rem] flex items-center gap-6">
                                    <div className="w-16 h-16 rounded-2xl border-4 border-white shadow-xl overflow-hidden shrink-0">
                                        <img src={selAlloc?.productId?.images?.[0]} className="w-full h-full object-cover" />
                                    </div>
                                    <div className="flex-1">
                                        <h4 className="text-sm font-black italic text-[#1E293B]">{selAlloc?.productId?.name}</h4>
                                        <p className="text-[10px] font-bold text-blue-500 uppercase tracking-widest mt-1">Wholesale Basis: ₹{Number(selAlloc?.sellerBasePrice || selAlloc?.productId?.basePrice).toLocaleString()}</p>
                                    </div>
                                </div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="relative">
                                            ...
                                            <input
                                                required
                                                type="number"
                                                min={selAlloc?.sellerMoq || selAlloc?.productId?.moq || 1}
                                                max={selAlloc?.allocatedStock}
                                                placeholder={`Min: ${selAlloc?.sellerMoq || selAlloc?.productId?.moq || 1} Units`}
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        {formData.retailPrice && selAlloc && (
                                            <div className="flex items-center justify-between px-2 pt-2">
                                                ...
                                                <p className={`text-[11px] font-black ${((Number(formData.retailPrice) / Number(selAlloc?.sellerBasePrice || selAlloc?.productId?.basePrice || 1)) - 1) * 100 > Number(selAlloc?.maxMargin)
                                                    ? 'text-rose-500' : 'text-emerald-600'
                                                    }`}>
                                                    {(((Number(formData.retailPrice) / Number(selAlloc?.sellerBasePrice || selAlloc?.productId?.basePrice || 1)) - 1) * 100).toFixed(1)}%
                                                </p>
                                            </div>
                                        )}
                                    </div>
                                ...
                                {((Number(formData.retailPrice) / Number(selAlloc?.sellerBasePrice || selAlloc?.productId?.basePrice)) - 1) * 100 > Number(selAlloc?.maxMargin) && (
                                ...
                                <button
                                    disabled={isSubmitting || ((Number(formData.retailPrice) / Number(selAlloc?.sellerBasePrice || selAlloc?.productId?.basePrice)) - 1) * 100 > Number(selAlloc?.maxMargin)}
                                    type="submit"
