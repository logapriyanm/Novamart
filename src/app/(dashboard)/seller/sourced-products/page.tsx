    ...
    return (
        <div className="space-y-8 animate-fade-in pb-12 text-slate-900">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black tracking-tight italic">Sourced <span className="text-[#067FF9]">Products</span></h1>
                    <p className="text-slate-400 font-bold uppercase tracking-widest text-sm mt-1">Manage Manufacturer Allocations & Retail Listings</p>
                </div>
                ...
                {isLoading ? (
                    ...
                    >
                        ...
                        <div className="p-8 space-y-6">
                            <div>
                                <h4 className="text-xl font-black italic text-slate-900 group-hover:text-[#067FF9] transition-colors">{item.productId?.name}</h4>
                            ...
                            <form onSubmit={handleSourceAndList} className="space-y-8">
                                <div className="p-6 bg-blue-50 border border-blue-100 rounded-[2rem] flex items-center gap-6">
                                    <div className="w-16 h-16 rounded-[10px] border-4 border-white shadow-xl overflow-hidden shrink-0">
                                        <img src={selAlloc?.productId?.images?.[0]} className="w-full h-full object-contain" />
                                    </div>
                                    <div className="flex-1">
                                        <h4 className="text-sm font-black italic text-slate-900">{selAlloc?.productId?.name}</h4>
                                        <p className="text-sm font-bold text-blue-500 uppercase tracking-widest mt-1">Wholesale Basis: ₹{Number(selAlloc?.sellerBasePrice || selAlloc?.productId?.basePrice).toLocaleString()}</p>
                                    </div>
                                </div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        {formData.retailPrice && selAlloc && (
                                            <div className="flex items-center justify-between px-2 pt-2">
                                                ...
                                                <p className={`text-sm font-black ${((Number(formData.retailPrice) / Number(selAlloc?.sellerBasePrice || selAlloc?.productId?.basePrice || 1)) - 1) * 100 > Number(selAlloc?.maxMargin)
