        ...
        <div className="space-y-10 pb-20 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <WhiteCard className="p-8 flex items-center justify-between">
                    <div>
                        <p className="text-sm font-black text-slate-400 pl-1 mb-1">Pending Approval</p>
            ...
            <div className="space-y-6">
                {isLoading ? (
                    <div className="p-20 text-center text-slate-300 font-black animate-pulse">Scanning database...</div>
                        ...
                        <div className="flex flex-col lg:flex-row gap-10">
                            ...
                            <div className="w-full lg:w-64 space-y-6 shrink-0">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="text-sm font-black text-slate-900 italic tracking-tight">{review.customerId?.name || 'Anonymous'}</p>
                                        <p className="text-sm font-black text-slate-400 mt-0.5">Customer</p>
                                    </div>
                                </div>
                            ...
                            <div className="flex-1 space-y-4">
                                ...
                                {review.type === 'SELLER' && (
                                    <div className="flex flex-wrap gap-6 px-2">
                                        <div className="text-sm font-black text-slate-400">Delivery: <span className="text-slate-800 ml-1">{review.deliveryRating || 'N/A'}</span></div>
                                        <div className="text-sm font-black text-slate-400">Packaging: <span className="text-slate-800 ml-1">{review.packagingRating || 'N/A'}</span></div>
                                        <div className="text-sm font-black text-slate-400">Support: <span className="text-slate-800 ml-1">{review.communicationRating || 'N/A'}</span></div>
                                    </div>
                                )}
                            </div>
                                ...
                                {review.status === 'PENDING' ? (
                                    <>
                                        <button
                                            ...
                                            className="w-full h-14 bg-emerald-500 text-white rounded-[10px] flex items-center justify-center gap-3 text-sm font-black shadow-lg shadow-emerald-500/20 hover:bg-emerald-600 transition-all font-mono"
                                        >
                                            <FaCheck className="w-3 h-3" /> Approve
                                        </button>
                                        <button
                                            onClick={() => handleModerate(review._id, review.type, 'REJECTED')}
                                            className="w-full h-14 bg-slate-100 text-slate-400 rounded-[10px] flex items-center justify-center gap-3 text-sm font-black hover:bg-rose-50 hover:text-rose-500 hover:border-rose-100 transition-all border border-transparent"
                                        >
                                            <FaTimes className="w-3 h-3" /> Hide
                                        </button>
                                    </>
                                ) : (
                                    <div className={`px-8 py-4 rounded-[10px] border ${review.status === 'APPROVED' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'} text-sm font-black tracking-[0.2em]`}>
                                        {review.status}
                                    </div>
                                )}
                            </div>
