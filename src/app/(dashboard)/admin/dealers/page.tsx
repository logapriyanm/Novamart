...
export default function DealerApprovalPanel() {
    ...
    const fetchDealers = async () => {
        setIsLoading(true);
        try {
            const result = await apiClient.get<any>('/admin/dealers');
            setDealers(result || []);
        } catch (error) {
    ...
    const handleAction = async (dealerId: string, verify: boolean) => {
        setIsVerifying(true);
        try {
            await apiClient.put(`/admin/dealers/${dealerId}/verify`, { isVerified: verify });
            setDealers(prev => prev.map(d => d.id === dealerId ? { ...d, isVerified: verify } : d));
        ...
        <div className="space-y-8 animate-fade-in pb-12 text-[#1E293B]">
            ...
            <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-black tracking-tight italic">Dealer <span className="text-[#10367D]">Onboarding</span></h1>
                    ...
                    <div className="px-5 py-2.5 bg-[#10367D]/5 border border-[#10367D]/10 rounded-xl">
                        <span className="text-sm font-black text-[#10367D]">{dealers.length} Total Registered</span>
                    </div>
                </div>
            </div>
                ...
                <div className="xl:col-span-7 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col min-h-[600px]">
                    <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                        ...
                        <div className="relative w-64">
                            ...
                            <input
                                type="text"
                                placeholder="Search by Business or GST..."
                                className="w-full bg-white border border-slate-100 rounded-xl py-2 pl-9 pr-4 text-xs font-bold focus:outline-none focus:bg-white transition-all"
                                value={searchQuery}
                    ...
                    <div className="divide-y divide-slate-50 flex-1">
                        {isLoading ? (
                            <div className="p-20 text-center text-xs font-black text-slate-300 uppercase tracking-widest">Accessing Retail Database...</div>
                        ) : filteredDealers.length === 0 ? (
                            <div className="p-20 text-center text-xs font-black text-slate-300 uppercase tracking-widest">No Dealers Found</div>
                        ) : (
                            filteredDealers.map((dlr) => (
                                <div
                                    key={dlr.id}
                                    onClick={() => setSelectedRequest(dlr)}
                                    className={`p-8 flex items-center justify-between hover:bg-[#10367D]/5 cursor-pointer transition-all ${selectedRequest?.id === dlr.id ? 'bg-[#10367D]/5 border-l-4 border-[#10367D]' : ''}`}
                                >
                                    <div className="flex items-center gap-6">
                                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-colors ${dlr.user?.status === 'SUSPENDED' ? 'bg-rose-50 text-rose-600' : dlr.isVerified ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>
                                            <FaStore className="w-7 h-7" />
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                <h4 className="text-base font-black text-[#1E293B]">{dlr.businessName || 'Unnamed Entity'}</h4>
                                                {dlr.isVerified && (
                                            ...
                                            <div className="flex items-center gap-3">
                                                <span className="text-[10px] font-black text-[#10367D] uppercase tracking-widest bg-white px-2 py-0.5 rounded border border-[#10367D]/10">GST: {dlr.gstNumber || 'N/A'}</span>
                                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1">
                                                    <FaMapMarkerAlt className="w-2.5 h-2.5" />
                                                    {dlr.city || 'Unknown Location'}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        {selectedRequest ? (
                            ...
                            >
                                <div className="flex items-center justify-between mb-10">
                                    <h3 className="text-xl font-black tracking-tight">Retail Governance</h3>
                                    <span className="text-[9px] font-black uppercase tracking-widest bg-[#10367D] px-3 py-1 rounded-full">Dealer ID: {selectedRequest.id?.slice(0, 8)}</span>
                                </div>
                                ...
                                <div className="space-y-8">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="p-6 bg-white/5 rounded-2xl border border-white/10 space-y-4">
                                            <div>
                                                <p className="text-[8px] font-black text-slate-500 uppercase mb-1">Business Address</p>
                                                <p className="text-xs font-bold leading-relaxed">{selectedRequest.businessAddress || 'No address provided'}</p>
                                            </div>
                                            <div className="grid grid-cols-2 gap-4">
                                                <div>
                                                    <p className="text-[8px] font-black text-slate-500 uppercase mb-1">City</p>
                                                    <p className="text-xs font-bold">{selectedRequest.city || 'N/A'}</p>
                                                </div>
                                                <div>
                                                    <p className="text-[8px] font-black text-slate-500 uppercase mb-1">Pincode</p>
                                                    <p className="text-xs font-bold">{selectedRequest.pincode || 'N/A'}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="grid grid-cols-1 gap-2">
                                            <div className="p-4 bg-white/5 rounded-2xl border border-white/10 flex items-center justify-between">
                                                <div>
                                                    <p className="text-[8px] font-black text-slate-500 uppercase mb-1">GST Registration</p>
                                                    <p className="text-xs font-black text-white">{selectedRequest.gstNumber || 'N/A'}</p>
                                                </div>
                                                <FaCheckCircle className="w-4 h-4 text-emerald-500" />
                                            </div>
                                            <div className="p-4 bg-white/5 rounded-2xl border border-white/10 flex items-center justify-between">
                                                <div>
                                                    <p className="text-[8px] font-black text-slate-500 uppercase mb-1">Bank Account</p>
                                                    <p className="text-xs font-black text-white">{selectedRequest.bankDetails ? 'Linked & Secured' : 'Not Provided'}</p>
                                                </div>
                                                {selectedRequest.bankDetails ? <FaShieldAlt className="w-4 h-4 text-[#10367D]" /> : <FaExclamationCircle className="w-4 h-4 text-rose-500" />}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="pt-8 space-y-4">
                                        <div className="grid grid-cols-2 gap-4">
                                            {selectedRequest.isVerified ? (
                                                ...
                                                >
                                                    {isVerifying ? 'Updating...' : 'Revoke License'}
                                                </button>
                                            ) : (
                                                <button
                                        ...
                                        <p className="text-[8px] font-black text-slate-500 text-center uppercase tracking-widest leading-relaxed px-4">
                                            Block operations will immediate disable all active inventory and pause open payout cycles.
                                        </p>
                                    </div>
                                </div>
                            </motion.div>
                            ...
                            <div className="h-[600px] border-2 border-dashed border-white/5 rounded-[3rem] flex flex-col items-center justify-center p-12 text-center text-slate-500">
                                <div className="w-20 h-20 rounded-full bg-white/5 flex items-center justify-center mb-6">
                                    <FaStore className="w-10 h-10 text-white/10" />
                                </div>
