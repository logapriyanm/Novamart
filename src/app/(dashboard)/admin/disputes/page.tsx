...
export default function DisputeCenter() {
    ...
    const handleResolve = async (orderId: string, resolution: 'REFUND' | 'REJECT') => {
        try {
            ...
            setDisputes(prev => prev.filter(d => (d._id || d.id) !== orderId));
        } catch (error) {
        ...
        <div className="space-y-8 animate-fade-in pb-12">
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-[10px] border border-slate-100 shadow-sm p-20 flex flex-col items-center text-center">
                    ...
                    <h3 className="text-lg font-black text-[#1E293B]">No Active Conflicts</h3>
                    <p className="text-sm font-bold text-slate-400 mt-2 max-w-md">
                ...
                <div className="bg-white rounded-[10px] border border-slate-100 shadow-sm overflow-hidden">
                    <div className="p-6 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between">
                        <h2 className="text-sm font-bold text-[#1E293B]">Active Disputes ({disputes.length})</h2>
                    </div>
                    <div className="divide-y divide-slate-50">
                        {disputes.map((dispute) => (
                            <div key={dispute._id || dispute.id} className="p-6 hover:bg-slate-50 transition-all flex items-center justify-between group">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="text-sm font-bold text-[#1E293B]">Order #{(dispute._id || dispute.id || '').slice(0, 8)}</h4>
                                        <p className="text-sm text-slate-500 mt-1 max-w-md line-clamp-1">{dispute.returnReason || 'No reason provided'}</p>
                                        <div className="flex items-center gap-2 mt-2">
                                            <span className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded text-sm font-bold">{dispute.sellerId?.businessName || dispute.seller?.businessName || 'Seller'}</span>
                                            <span className="text-sm font-bold text-slate-300">•</span>
                                            <span className="text-sm font-bold text-slate-400">₹{dispute.totalAmount}</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2">
                                    <button
                                        onClick={() => handleResolve(dispute._id || dispute.id, 'REJECT')}
                                        className="px-4 py-2 border border-slate-200 text-slate-600 rounded-[10px] text-sm font-bold hover:bg-slate-50 transition-all"
                                    >
                                        Reject
                                    </button>
                                    <button
                                        onClick={() => handleResolve(dispute._id || dispute.id, 'REFUND')}
                                        className="px-4 py-2 bg-black text-white rounded-[10px] text-sm font-bold hover:bg-slate-800 transition-all shadow-lg shadow-blue-500/10"
                                    >
                                        Approve Refund
                                    </button>
                                </div>
