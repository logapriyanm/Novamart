            ...
            <div className="bg-white rounded-[10px] border border-slate-100 shadow-sm overflow-hidden">
                <div className="p-10 border-b border-slate-50 bg-[#10367D] text-white flex items-center justify-between">
                    ...
                    <div className="p-4 bg-white/10 rounded-[10px] backdrop-blur-sm border border-white/10">
                        <p className="text-sm font-black text-blue-200">Total Held Value</p>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-slate-50">
                                <th className="px-10 py-5 text-sm font-black text-slate-400">Transaction ID</th>
                                <th className="px-10 py-5 text-sm font-black text-slate-400">Amount</th>
                                <th className="px-10 py-5 text-sm font-black text-slate-400">Beneficiary (Dealer)</th>
                                <th className="px-10 py-5 text-sm font-black text-slate-400 text-right">Settlement Actions</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                            {loading ? (
                                <tr><td colSpan={4} className="p-10 text-center text-sm font-bold text-slate-400">Loading Vault...</td></tr>
                            ) : orders.length === 0 ? (
                                <tr><td colSpan={4} className="p-10 text-center text-sm font-bold text-slate-400">No Active Holds. All funds settled.</td></tr>
                            ) : orders.map((order) => (
                                    ...
                                    <td className="px-10 py-6 text-right">
                                        <div className="flex items-center justify-end gap-3">
                                            <button
                                                ...
                                                className="px-4 py-2 bg-rose-50 text-rose-600 rounded-[10px] text-sm font-black hover:bg-rose-500 hover:text-white transition-all"
                                            >
                                                <FaHistory className="inline mr-1" /> Refund
                                            </button>
                                            <button
                                                onClick={() => handleRelease(order.id)}
                                                className="px-4 py-2 bg-emerald-50 text-emerald-600 rounded-[10px] text-sm font-black hover:bg-emerald-500 hover:text-white transition-all"
                                            >
