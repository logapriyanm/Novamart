    ...
    if (!seller) {
        return (
            <div className="min-h-screen pt-40 text-center bg-[#F9FAFB]">
                ...
                <Link href="/products" className="text-primary font-black mt-4 block uppercase text-sm tracking-widest">Return to Market</Link>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen pt-40 pb-20 bg-[#F9FAFB]">
            <div className="max-w-7xl mx-auto px-6">
                {/* Back Button */}
                <Link href="/products" className="inline-flex items-center gap-3 text-sm font-black text-slate-400 uppercase tracking-[0.2em] mb-12 hover:text-black transition-all">
                        ...
                        <div className="flex-1 space-y-6">
                            ...
                            <div className="flex flex-wrap gap-10 pt-4 border-t border-slate-50">
                                ...
                                <div className="flex items-center gap-4">
                                    <div className="flex text-amber-400 gap-1 bg-amber-50 px-4 py-2 rounded-[10px] border border-amber-100/50">
                                    ...
                                    <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Based on {seller.stats.reviewCount} Ratings</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex border-b border-border bg-muted/20">
                        {['Products', 'Reviews'].map((tab) => (
                            <button
                                ...
                                className={`px-12 py-7 text-sm font-black uppercase tracking-[0.3em] transition-all relative ${activeTab === tab ? 'text-primary' : 'text-muted-foreground hover:text-foreground'}`}
                            >
                                ...
                                >
                                    {seller.products.map((product: any) => (
                                        <Link
                                            ...
                                            className="group bg-white rounded-[10px] border border-slate-100 p-5 hover:shadow-2xl transition-all hover:scale-[1.02]"
                                        >
                                            <div className="aspect-square bg-slate-50 rounded-[10px] overflow-hidden mb-6 relative p-4">
                                                <img
                                            ...
                                            <div className="space-y-3">
                                                <p className="text-xs font-black text-primary uppercase tracking-widest">{product.category}</p>
                                                ...
                                                <div className="pt-3 border-t border-slate-50 flex items-center justify-between">
                                                    ...
                                                    <div className="w-8 h-8 rounded-[10px] bg-slate-50 flex items-center justify-center text-slate-300 group-hover:bg-primary group-hover:text-white transition-all shadow-sm">
                                        ...
                                        <div key={review._id} className="bg-slate-50/50 p-8 rounded-[1.5rem] border border-slate-100 relative group hover:bg-white hover:shadow-xl transition-all">
                                            <div className="flex items-center justify-between mb-6">
                                                <div className="flex items-center gap-5">
                                                    <div className="w-14 h-14 rounded-[10px] bg-white border border-slate-100 shadow-sm flex items-center justify-center font-black text-slate-400 text-sm italic uppercase">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-xs font-black text-slate-400 uppercase tracking-widest mt-1">Verified Experience</p>
                                                    </div>
                                                </div>
                                                <div className="flex text-amber-400 gap-1 bg-white px-5 py-2.5 rounded-[10px] border border-slate-100 shadow-sm">
                                                    {[...Array(5)].map((_, i) => (
                                            ...
                                            <div className="absolute top-8 right-8 text-xs font-bold text-slate-300 uppercase italic opacity-0 group-hover:opacity-100 transition-opacity">
