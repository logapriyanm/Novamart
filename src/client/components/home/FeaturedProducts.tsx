    ...
    return (
        <div className="max-w-7xl mx-auto px-6 mt-20 mb-20">
            <div className="text-center mb-12">
                ...
                <h3 className="text-3xl font-black text-foreground tracking-tight">Featured Products</h3>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-[300px]">
                {featuredApplianceProducts.map((product) => (
                    <div key={product.id} className={`group relative bg-surface rounded-3xl overflow-hidden hover:shadow-xl hover:shadow-primary/5 border border-foreground/5 transition-all duration-500 ${product.cols} ${product.rows}`}>
                        ...
                        <div className="absolute inset-0 p-8 flex flex-col h-full pointer-events-none z-20">
                            ...
                            <div className={`relative flex flex-col ${product.variant === 'wide-image-right' || product.variant === 'small-image-right' ? 'w-1/2 justify-center h-full' : product.variant === 'tall-image-top' ? 'mt-auto' : ''}`}>
                                {product.verified && (
                                    <div className="flex items-center gap-1.5 mb-2">
                                        ...
                                        <span className="text-[9px] font-black text-primary uppercase tracking-widest">Verified Seller</span>
                                    </div>
                                )}
                                <h4 className="text-xl font-black text-foreground leading-none mb-2">{product.name}</h4>
                                <div className="flex items-center gap-2 mb-3">
                                    <div className="flex items-center gap-0.5">
                                        {[...Array(5)].map((_, i) => (
                                            <FaStar key={i} className={`w-3 h-3 ${i < Math.floor(product.rating) ? 'text-amber-400' : 'text-foreground/10'}`} />
                                        ))}
                                    </div>
                                    <span className="text-[10px] font-bold text-foreground/40">{product.rating}</span>
                                </div>
                            ...
                            <div className="absolute top-0 left-0">
                                {product.tag && (
                                    <span className="bg-primary text-background px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest shadow-lg shadow-primary/20">{product.tag}</span>
                                )}
                            </div>
                        </div>
                        ...
                        {/* Action Button */}
                        <button className="absolute bottom-6 right-6 z-30 w-10 h-10 bg-background rounded-full flex items-center justify-center shadow-lg hover:bg-primary group/btn transition-colors border border-foreground/5">
                            <FaHeart className="w-4 h-4 text-foreground/20 group-hover/btn:text-background transition-colors" />
                        </button>
                    </div>
                ))}
            </div>
