    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-between mb-4">
                {discount ? (
                    <span className="bg-black text-white text-xs font-bold px-2 py-1 rounded-[10px]">
                        {discount}%
                    </span>
                ) : <div />}
            ...
            <div className="space-y-2 mb-6">
                <div className="flex items-baseline justify-between">
                    <div className="flex items-baseline gap-2">
                        ...
                        {originalPrice && (
                            <span className="text-sm text-foreground/30 font-bold line-through">₹{originalPrice}</span>
                        )}
                    </div>
                    <div className="flex items-center gap-1">
                        <span className="text-sm font-black text-foreground/70">{rating}</span>
                        <Star className="w-3 h-3 fill-primary text-primary" />
                    </div>
                </div>
                <h3 className="text-sm font-bold text-foreground leading-tight line-clamp-1">{name}</h3>
                <p className="text-sm font-bold text-primary/40">{category}</p>
            </div>
            ...
            <div className="mt-auto pt-6 border-t border-foreground/5 grid grid-cols-3 gap-2 text-center">
                <div className="flex flex-col">
                    ...
                    <span className="text-xs font-bold text-foreground/30">Stocks</span>
                </div>
                <div className="flex flex-col border-x border-foreground/5">
                    <span className="text-sm font-bold text-foreground">{orders}</span>
                    <span className="text-xs font-bold text-foreground/30">Orders</span>
                </div>
                <div className="flex flex-col">
                    <span className="text-sm font-bold text-foreground whitespace-nowrap">{publishDate}</span>
                    <span className="text-xs font-bold text-foreground/30">Publish</span>
                </div>
            </div>
        </motion.div>
    );
