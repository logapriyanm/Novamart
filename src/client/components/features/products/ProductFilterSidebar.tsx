...
sidebarCategories.forEach((cat) => {
  subCategoriesMapFallback[cat.id] = cat.subsections.flatMap((sub) =>
    ...
    <aside className="bg-white border border-slate-200 rounded-[10px] shadow-sm flex flex-col divide-y divide-slate-100 overflow-hidden">
      ...
      <div className="p-4 xs:p-5">
        <button
          ...
          className={`w-full flex items-center justify-between p-3 rounded-[10px] border transition-all ${filters.verifiedOnly ? "bg-black border-black text-white" : "bg-white border-slate-100 text-slate-600 hover:border-slate-300"}`}
        >
          <div className="flex items-center gap-2">
            <FaShieldAlt
              className={`w-4 h-4 ${filters.verifiedOnly ? "text-white" : "text-slate-400"}`}
            />
      ...
      <div className="p-4 xs:p-5">
        ...
        >
          ...
          <FaChevronDown
            className={`w-3 h-3 text-slate-300 transition-transform duration-300 ${openSections.includes("categories") ? "rotate-180" : ""}`}
          />
        </button>
            ...
            {mainCategories.map((cat) => (
              <div key={cat.slug} className="space-y-1">
                <button
                  ...
                  className={`w-full flex items-center justify-between py-2 px-3 rounded-[8px] transition-all text-left ${currentCategory === cat.slug ? "bg-black text-white shadow-md" : "hover:bg-slate-50 text-slate-600"}`}
                >
                  <span className="text-sm font-semibold">{cat.name}</span>
                  {cat.count && (
                    <span
                      className={`text-sm font-bold ${currentCategory === cat.slug ? "text-white/60" : "text-slate-300"}`}
                    >
                      {cat.count}
                    </span>
                  )}
                ...
                {currentCategory === cat.slug &&
                  subCategoriesMapFallback[cat.slug] && (
      ...
      <div className="p-4 xs:p-5 overflow-hidden">
        ...
        >
          ...
          <FaChevronDown
            className={`w-3 h-3 text-slate-300 transition-transform duration-300 ${openSections.includes("price") ? "rotate-180" : ""}`}
          />
        </button>
      ...
      <div className="p-4 xs:p-5 overflow-hidden">
        ...
        >
          ...
          <FaChevronDown
            className={`w-3 h-3 text-slate-300 transition-transform duration-300 ${openSections.includes("brands") ? "rotate-180" : ""}`}
          />
        </button>
            ...
            <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1 thin-scrollbar">
              {filteredBrands.length > 0 ? (
                filteredBrands.map((brand) => (
                  ...
                  >
                    <div
                      className={`w-4 h-4 rounded-[4px] border transition-all flex items-center justify-center ${filters.brands.includes(brand) ? "bg-black border-black" : "border-slate-200 bg-white group-hover:border-slate-400"}`}
                    >
                      {filters.brands.includes(brand) && (
                        <FaCheckSquare className="text-white w-2.5 h-2.5" />
                      )}
                    </div>
                    <span
                      className={`text-sm font-medium ${filters.brands.includes(brand) ? "text-slate-900" : "text-slate-500 group-hover:text-slate-700"}`}
                    >
                      {brand}
                    </span>
                  </button>
      ...
      <div className="p-4 xs:p-5">
        ...
        >
          ...
          <FaChevronDown
            className={`w-3 h-3 text-slate-300 transition-transform duration-300 ${openSections.includes("rating") ? "rotate-180" : ""}`}
          />
        </button>
        {openSections.includes("rating") && (
          <div className="space-y-3 pt-1">
            {[4, 3, 2].map((rating) => (
              ...
              >
                <div
                  className={`w-4 h-4 rounded-full border-2 transition-all flex items-center justify-center ${filters.rating === rating ? "border-black bg-white" : "border-slate-200 group-hover:border-slate-400"}`}
                >
                ...
                <div className="flex items-center gap-0.5">
                  {[...Array(5)].map((_, i) =>
                    i < rating ? (
                      ...
                      <FaRegStar
                        key={i}
                        className="w-2.5 h-2.5 text-slate-200"
                      />
                    ),
                  )}
                </div>
                <span
                  className={`text-sm font-medium ${filters.rating === rating ? "text-slate-900" : "text-slate-400"}`}
                >
                  & Above
                </span>
              </button>
