...
export default function ChatRoom({ negotiationId, userRole }: ChatRoomProps) {
    ...
    const sendMessage = async (offerDetails?: any) => {
        ...
        try {
            ...
            if (offerDetails && chatId) {
                socket.emit('chat:message', {
                    chatId,
                    message: offerDetails.note || `Proposed ${offerDetails.quantity} units at ₹${offerDetails.price}`,
                    messageType: 'OFFER',
    ...
    const isLocked = ['ACCEPTED', 'REJECTED', 'ORDER_FULFILLED'].includes(negotiation.status);
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-px bg-slate-100 overflow-hidden rounded-[10px] border border-slate-200 shadow-xl">
                ...
                <aside className="order-2 lg:order-1 lg:col-span-3 bg-white flex flex-col overflow-y-auto max-h-[300px] lg:max-h-full border-t lg:border-t-0 border-slate-100">
                    <div className="p-6 border-b border-slate-50">
                        <button onClick={() => router.back()} className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-primary transition-colors mb-6">
                            <FaArrowLeft /> Back
                        </button>
                        <h2 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Deal Context</h2>
                        <div className="flex lg:flex-col gap-4 items-center sm:items-start">
                            <div className="aspect-square w-24 lg:w-full bg-slate-50 rounded-[10px] border border-slate-100 flex items-center justify-center p-2 lg:p-4 mb-0 lg:mb-4 shrink-0">
                                {negotiation.productId?.images?.[0] ? (
                                    <img src={negotiation.productId.images[0]} alt="" className="max-w-full max-h-full object-contain mix-blend-multiply" />
                                ) : (
                                    <FaBoxOpen className="w-8 h-8 lg:w-12 lg:h-12 text-slate-200" />
                                )}
                            </div>
                            <div>
                                <h3 className="text-sm lg:text-lg font-black tracking-tight leading-tight">{negotiation.productId?.name}</h3>
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Category • {negotiation.productId?.category}</p>
                            </div>
                        </div>
                    </div>

                    <div className="p-6 space-y-6 hidden lg:block">
                        <ContextItem icon={FaCoins} label="Base Price" value={`₹${negotiation.productId?.basePrice || 0}`} />
                        <ContextItem icon={FaBoxOpen} label="Min. Order Qty" value={`${negotiation.productId?.moq || 0} units`} />
                        <div className="pt-4 border-t border-slate-50">
                            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">{isDealer ? 'Manufacturer' : 'Dealer Account'}</h4>
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="flex items-center gap-1.5">
                                        <p className="text-xs font-black">{isDealer ? partner?.companyName : partner?.businessName}</p>
                                        <FaShieldAlt className="text-emerald-500 w-3 h-3" />
                                    </div>
                                    <p className="text-[9px] font-bold text-slate-400 uppercase">{partner?.city}, {partner?.state}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <main className="order-1 lg:order-2 lg:col-span-6 bg-[#F8FAFC] flex flex-col overflow-hidden relative border-x border-slate-100 h-[60vh] lg:h-full">
                    <div className="p-4 bg-white border-b border-slate-100 flex items-center justify-between z-10 shadow-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest hidden xs:inline">Active Negotiation</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Secured Channel</span>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 lg:p-6 space-y-6 lg:space-y-8 custom-scrollbar">
                        ...
                        {messages.map((m, i) => (
                            <MessageBubble
                                key={m._id || i}
                                msg={m}
                                isMe={(isDealer && m.senderRole === 'DEALER') || (!isDealer && m.senderRole === 'MANUFACTURER')}
                                onAccept={() => handleStatusUpdate('ACCEPTED')}
                                onReject={() => handleStatusUpdate('REJECTED')}
                                showActions={!isLocked && !((isDealer && m.senderRole === 'DEALER') || (!isDealer && m.senderRole === 'MANUFACTURER'))}
                            />
                        ))}
                        <div ref={chatEndRef} />
                    </div>
                        ...
                        {isLocked ? (
                            <div className="bg-slate-50 p-4 rounded-[10px] border border-dashed border-slate-200 text-center">
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-loose">
                                    Negotiation concluded with status: <span className="text-primary">{negotiation.status}</span>
                                    <br />Terms are now finalized and legally binding.
                                </p>
                            </div>
                        ) : (
                            <div className="flex flex-col gap-4">
                                ...
                                <div className="flex items-center gap-2 px-2 hidden sm:flex">
                                    ...
                                    <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Once an offer is accepted, the deal terms move to contract allocation.</p>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <aside className="order-3 lg:col-span-3 bg-white flex flex-col max-h-[400px] lg:max-h-full overflow-y-auto border-t lg:border-t-0 border-slate-100">
                    <div className="p-6 border-b border-slate-50 bg-slate-50/30">
                        <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6">Negotiation Summary</h2>
                        <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-1 gap-4">
                            <SummaryCard label="Current Best Price" value={`₹${negotiation.currentOffer}`} icon={FaCoins} highlighted />
                            <SummaryCard label="Target Quantity" value={`${negotiation.quantity} units`} icon={FaBoxOpen} />
                            <SummaryCard label="Total Deal Value" value={`₹${(negotiation.currentOffer * negotiation.quantity).toLocaleString()}`} icon={FaHandshake} />
                        </div>
                    </div>
                    ...
                    <div className="flex-1 p-6 overflow-y-auto hidden lg:block">
                        ...
                        <div className="space-y-4 relative before:absolute before:left-2 before:top-2 before:bottom-0 before:w-px before:bg-slate-100">
                            {messages.filter(m => m.messageType === 'SYSTEM' || m.messageType === 'OFFER').slice(-5).map((m, idx) => (
                                <div key={idx} className="relative pl-6">
                                    ...
                                    <p className="text-[10px] font-bold text-slate-700 leading-tight">{m.message}</p>
                                    <p className="text-[8px] font-bold text-slate-400 uppercase mt-0.5">{new Date(m.createdAt).toLocaleDateString()}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="p-6 border-t border-slate-50">
                        {negotiation.status === 'ACCEPTED' ? (
                            <button
                                onClick={() => router.push(isDealer ? '/dealer/orders' : '/manufacturer/orders')}
...
function SummaryCard({ label, value, icon: Icon, highlighted }: any) {
    return (
        <div className={`p-4 rounded-[10px] border ${highlighted ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20' : 'bg-white text-slate-800 border-slate-100 shadow-sm'}`}>
            <div className="flex justify-between items-start mb-1">
                <p className={`text-[8px] font-black uppercase tracking-[0.2em] ${highlighted ? 'text-white/60' : 'text-slate-400'}`}>{label}</p>
                <Icon className={`w-3 h-3 ${highlighted ? 'text-white/40' : 'text-slate-200'}`} />
            </div>
...
function StatusBadge({ status }: { status: string }) {
    ...
    return (
        <div className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-[10px] border border-slate-100">
            ...
            <span className="text-[10px] font-black text-slate-800 uppercase tracking-widest">{status.replace('_', ' ')}</span>
        </div>
    );
}
    ...
    if (msg.messageType === 'OFFER') {
        return (
            <div className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[400px] w-full bg-white border-2 border-primary rounded-[10px] overflow-hidden shadow-xl ${isMe ? 'border-primary/40' : 'border-primary'}`}>
                    <div className="bg-primary p-4 text-white flex justify-between items-center">
                        <div className="flex items-center gap-2">
                            <FaCoins className="w-3.5 h-3.5" />
                            <span className="text-[10px] font-black uppercase tracking-widest">Business Offer</span>
                        </div>
                        <span className="text-[9px] font-bold opacity-70 italic">{new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                    <div className="p-5 space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-slate-50 p-3 rounded-[10px] border border-slate-100">
                                <p className="text-[8px] font-black text-slate-400 uppercase mb-1">Proposed Price</p>
                                <p className="text-lg font-black text-slate-800">₹{msg.metadata?.price}</p>
                            </div>
                            <div className="bg-slate-50 p-3 rounded-[10px] border border-slate-100">
                                <p className="text-[8px] font-black text-slate-400 uppercase mb-1">Target Quantity</p>
                                <p className="text-lg font-black text-slate-800">{msg.metadata?.quantity} units</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-2 text-slate-500">
                            <FaCalendarAlt className="w-3 h-3" />
                            <p className="text-[9px] font-bold uppercase tracking-widest">Timeline: {msg.metadata?.timeline}</p>
                        </div>
                        {msg.message && <p className="text-xs font-medium text-slate-600 bg-slate-50 p-3 rounded-[10px] italic">"{msg.message}"</p>}

                        {showActions && (
                            <div className="flex gap-2 pt-2">
                                <button onClick={onReject} className="flex-1 py-3 bg-white border border-rose-100 text-rose-500 rounded-[10px] text-[9px] font-black uppercase tracking-widest hover:bg-rose-50 transition-all">Reject</button>
                                <button onClick={onAccept} className="flex-1 py-3 bg-primary text-white rounded-[10px] text-[9px] font-black uppercase tracking-widest shadow-lg shadow-primary/20 hover:scale-[1.02] transition-all">Accept Offer</button>
                            </div>
                        )}
                        {!showActions && isMe && (
                            <div className="text-center pt-2">
                                <span className="text-[9px] font-black text-primary/60 uppercase tracking-widest italic">Awaiting Response</span>
                            </div>
                        )}
                    </div>
                </div>
    ...
    return (
        <div className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[70%] group flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                <div className={`px-5 py-3.5 rounded-[10px] shadow-sm ${isMe ? 'bg-slate-900 text-white rounded-tr-none' : 'bg-white text-slate-800 rounded-tl-none border border-slate-100'}`}>
                    <p className={`text-[8px] font-black uppercase tracking-widest mb-1 opacity-40`}>{isMe ? 'Internal' : msg.senderRole}</p>
