            ...
            <div className="flex-1 space-y-8">
                ...
                <div className="bg-white rounded-[10px] border border-slate-100 shadow-sm p-8">
                    <div className="flex items-center gap-3 mb-8">
                        <div className="w-8 h-8 bg-[#0F6CBD] text-white rounded-[10px] flex items-center justify-center shadow-lg shadow-blue-500/20">
                                ...
                                {spec.type === 'select' && spec.options ? (
                                    <div className="relative">
                                        <select
                                            ...
                                            className="w-full bg-slate-50 border border-slate-200 rounded-[10px] px-4 py-3 text-sm font-bold text-[#1E293B] focus:outline-none focus:border-[#0F6CBD] transition-all appearance-none"
                                        >
                    ...
                    <div className="mt-8 p-4 bg-slate-50 rounded-[10px] flex items-center justify-between border border-slate-200 cursor-pointer hover:border-[#0F6CBD] transition-all" onClick={() => updateProductData({ isSmart: !productData.isSmart })}>
                        <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-[10px] flex items-center justify-center transition-colors ${productData.isSmart ? 'bg-[#0F6CBD] text-white' : 'bg-slate-200 text-slate-400'}`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50/50 p-6 rounded-[10px] border border-dashed border-slate-200">
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    className="w-full bg-white border border-slate-200 rounded-[10px] px-4 py-3 text-sm font-bold text-[#1E293B] focus:outline-none focus:border-[#0F6CBD]"
                                />
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-slate-400">Value</label>
                                <div className="flex gap-2">
                                    <input
                                        ...
                                        className="flex-1 bg-white border border-slate-200 rounded-[10px] px-4 py-3 text-sm font-bold text-[#1E293B] focus:outline-none focus:border-[#0F6CBD]"
                                    />
                                    <button
                                        onClick={addSpec}
                                        disabled={!newKey || !newValue}
                                        className="p-3 bg-[#0F6CBD] text-white rounded-[10px] shadow-lg shadow-blue-500/20 hover:bg-[#0F6CBD]/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                                    >
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4">
                        {productData.images && productData.images.map((img: string, index: number) => (
                            <div key={index} className={`relative group aspect-square rounded-xl overflow-hidden border bg-white ${index === 0 ? 'border-[#0F6CBD]' : 'border-slate-100'}`}>
                                {index === 0 && (
                                    <span className="absolute top-1 left-1 bg-[#0F6CBD] text-white text-[6px] font-black px-1.5 py-0.5 rounded-[10px] shadow-sm z-10 uppercase tracking-wider">Cover</span>
                                )}
                                ...
                                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1">
                                    <button
                                        ...
                                        className="p-1.5 bg-white text-rose-500 rounded-[10px] hover:bg-rose-50"
                                    >
                        ...
                        <div className="relative group">
                            ...
                            />
                            <div className={`aspect-square rounded-[10px] border-2 border-dashed flex flex-col items-center justify-center transition-all ${isUploading ? 'bg-slate-50 border-blue-200 text-blue-400' : 'border-slate-200 text-slate-300 group-hover:text-[#0F6CBD] group-hover:border-[#0F6CBD] group-hover:bg-slate-50'}`}>
                                {isUploading ? (
