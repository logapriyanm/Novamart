    ...
    if (isLoading) {
        return (
            <div className="flex items-center justify-center min-h-[400px]">
                <div className="animate-pulse text-slate-400 font-bold">Loading Product Data...</div>
            </div>
        );
    }
        ...
        <div className="max-w-7xl mx-auto space-y-8 pb-32 animate-fade-in relative min-h-screen">
            ...
            <div className="flex flex-col gap-1 text-[#1E293B]">
                <nav className="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">
                    ...
                    <span>/</span>
                    <span className="text-[#1E293B]">{productId ? 'Edit Product' : 'Add New Product'}</span>
                </nav>
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-black tracking-tight italic">{productId ? 'Edit Product' : 'Create New Product'}</h1>
                    </div>
                    {isVerified && (
                        <div className="flex items-center gap-3 px-6 py-3 bg-emerald-50 border border-emerald-100 rounded-[10px]">
                            ...
                            <div>
                                <p className="text-[10px] font-bold text-emerald-800 uppercase tracking-widest">Verified Account</p>
                ...
                <div className="flex items-center justify-between px-12 relative">
                    ...
                    {steps.map((step) => (
                        <div key={step.id} className="flex flex-col items-center gap-3 cursor-pointer group" onClick={() => setCurrentStep(step.id)}>
                            <div className={`w-10 h-10 rounded-[10px] flex items-center justify-center text-sm font-bold transition-all border-2 ${currentStep > step.id
                            ...
                            <span className={`text-[10px] font-bold uppercase tracking-widest transition-colors ${currentStep >= step.id ? 'text-black' : 'text-foreground/20'
                                }`}>
                                {step.name}
                            </span>
                        </div>
                ...
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        {currentStep < steps.length ? (
                            ...
                            >
                                {isSubmitting ? 'Saving...' : productId ? 'Update Product' : isVerified ? 'Publish Instantly' : 'Submit for Approval'}
                                {productId ? <FaSave className="w-3 h-3" /> : <FaRocket className="w-3 h-3" />}
                            </button>
                        )}
                    </div>
                </div>
