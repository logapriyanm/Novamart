    ...
    >
      ...
      <div className="absolute top-4 left-4 z-10 flex gap-2">
        {status === "Price Dropped" && (
          <span className="px-2 py-1 bg-green-500/10 text-green-600 text-xs font-bold rounded-[5px]">
            Price Dropped
          </span>
        )}
        {status === "Low Stock" && (
          <span className="px-2 py-1 bg-amber-500/10 text-amber-600 text-xs font-bold rounded-[5px]">
            Low Stock
          </span>
        )}
      </div>
      ...
      <div className="flex-1 flex flex-col">
        <div className="mb-2">
          ...
          <Link href={`/products/${id}`} className="block">
            <h3 className="text-sm font-bold text-foreground leading-snug hover:text-foreground/60 transition-colors line-clamp-2">
              {name}
            </h3>
          </Link>
        </div>
