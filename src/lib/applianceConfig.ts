/**
 * Master Configuration for Home Appliance Categories & Filters
 * This file defines the relationship between categories, sub-categories, 
 * and their valid technical specifications/filters.
 */

export interface FilterOption {
    id: string;
    label: string;
    type: 'select' | 'range' | 'boolean' | 'number' | 'text';
    options?: string[];
    unit?: string;
    min?: number;
    max?: number;
...
export const GLOBAL_FILTERS: FilterOption[] = [
    ...
    { id: 'brand', label: 'Brand', type: 'select', options: [] }, // Dynamic from DB
...
export const APPLIANCE_METADATA: Record<string, CategoryConfig> = {
...
export const getFiltersBySubCategory = (subCategoryName: string): FilterOption[] => {
...
export const getSubCategoryLabel = (subCategoryName: string): string => {
    const slug = subCategoryName.toLowerCase().replace(/\s+/g, '-');
    for (const catId in APPLIANCE_METADATA) {
        const subCats = APPLIANCE_METADATA[catId].subCategories;
        if (subCats[slug]) {
            return subCats[slug].label;
        }
    }
