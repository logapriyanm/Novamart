...
const socket = io('http://localhost:5000'); // Use env in production
...
export default function ChatWidget({ productId, dealerId, dealerName }: { productId: string, dealerId: string, dealerName: string }) {
    ...
    const startChat = async () => {
        ...
        try {
            ...
            const histRes = await fetch(`http://localhost:5000/api/chat/${data._id}/messages`, {
    ...
    const sendMessage = () => {
        ...
        const payload = {
            chatId,
            message: input,
            senderId: 'mock-user-id', // From context/localstorage
            senderRole: 'CUSTOMER'
        };
