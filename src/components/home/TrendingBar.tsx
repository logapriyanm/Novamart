...
export default function TrendingBar() {
    return (
        <div className="max-w-7xl mx-auto px-6 mt-18">
            ...
            <div id="trending-appliances" className="bg-white rounded-[2rem] p-8 lg:p-12 shadow-2xl shadow-slate-200/50 border border-slate-200/60 relative -mt-28 z-30">
                <div className="flex items-center justify-between mb-12">
                    <div>
                        <h2 className="text-3xl font-black text-slate-800 tracking-tight">Trending Appliances <span className="text-[#74B4DA] font-medium text-lg ml-2 italic">in Your Location</span></h2>
                        <p className="text-slate-400 text-sm font-bold uppercase tracking-widest mt-2">Verified Direct Factory Supply</p>
                    </div>
                    <Link href="/products" className="group flex items-center gap-2 text-[#74B4DA] text-[10px] font-black uppercase tracking-widest border-2 border-[#10367D]/10 px-6 py-3 rounded-2xl hover:bg-[#10367D] hover:text-white transition-all">
                        Explore All
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-8">
                    {categories.map((cat, i) => (
                        <Link key={i} href={`/products?q=${cat.name}`} className="group flex flex-col items-center text-center">
                            <div className="w-full aspect-square rounded-[.55rem] bg-slate-50 border border-slate-100 overflow-hidden mb-4 group-hover:shadow-xl group-hover:shadow-slate-200 transition-all p-4">
                                <img
                                    src={cat.image}
                                    alt={cat.name}
                            ...
                            <span className="text-xs font-black text-slate-600 group-hover:text-[#74B4DA] transition-colors">{cat.name}</span>
                        </Link>
                    ))}
                </div>
            </div>
