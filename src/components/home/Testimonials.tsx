    ...
    return (
        <div className="max-w-7xl mx-auto px-6 mt-32 mb-32">
            <div className="text-center mb-16">
                <h2 className="text-3xl lg:text-4xl font-black text-slate-800 tracking-tight">Trusted by Industry Leaders</h2>
                <p className="text-slate-500 font-medium mt-4 max-w-2xl mx-auto">See what our partners and customers are saying about their experience with Novamart.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 auto-rows-auto">
                {testimonials.map((item) => (
                    <div key={item.id} className={`bg-white rounded-[2rem] p-8 shadow-xl shadow-slate-200/50 border border-slate-100 hover:-translate-y-1 transition-transform duration-300 relative overflow-hidden ${item.cols} ${item.rows}`}>
                        ...
                        {item.variant === 'service-card' && (
                            <div className="flex flex-col items-center text-center h-full">
                                <div className="relative mb-6">
                                    ...
                                    <div className="w-14 h-14 rounded-full p-1 bg-white border border-slate-100 shadow-sm">
                                ...
                                <p className="text-[10px] text-slate-400 leading-relaxed mb-4">{item.text}</p>
                            ...
                            <div className="h-full flex flex-col">
                                <div className="flex items-start justify-between mb-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <FaQuoteRight className="text-2xl text-slate-800" />
                                    </div>
                                </div>
                                <div className="mt-2 mb-6">
                                    <span className="text-[10px] font-bold text-slate-400 block mb-1">{item.author}</span>
                                    <span className="text-[8px] font-bold text-slate-300 uppercase tracking-wider">{item.role}</span>
                                </div>
                                <h3 className="text-sm font-black text-slate-800 mb-2">{item.title}</h3>
                                <p className="text-[10px] text-slate-400 leading-relaxed">{item.text}</p>
                        ...
                        {item.variant === 'photo-split' && (
                            <div className="flex items-center gap-6 h-full">
                                <div className="flex-1">
                                    <FaQuoteLeft className="text-xl text-slate-800 mb-3" />
                                    <h3 className="text-sm font-black text-slate-800 mb-2">{item.title}</h3>
                                    <p className="text-[10px] text-slate-400 leading-relaxed mb-4">{item.text}</p>
                                    <div>
                                        <p className="text-[10px] font-bold text-slate-800">{item.author}</p>
                                        <p className="text-[8px] text-slate-400">{item.role}</p>
                                    </div>
                                </div>
                        ...
                        {item.variant === 'wide-likes' && (
                            <div className="flex flex-col justify-center h-full relative">
                                <div className="absolute top-0 right-0 bg-slate-50 px-3 py-1 rounded-bl-2xl flex items-center gap-1">
                                    <FaHeart className="text-xs text-rose-500" />
                                    <span className="text-[10px] font-bold text-slate-600">{item.likes} Like</span>
                                </div>
                                <div className="flex items-start gap-8 px-4">
                                    <div className="flex-1 text-center">
                                        <h3 className="text-xl font-black text-slate-800 mb-3">{item.title}</h3>
                                        <p className="text-xs text-slate-400 leading-relaxed max-w-lg mx-auto mb-6">{item.text}</p>
                                        <span className="text-xs font-bold text-slate-800 block">{item.author}</span>
                                    </div>
                                </div>
                                <div className="flex justify-center mt-6 -space-x-2">
                                    <img src={item.image} className="w-8 h-8 rounded-full border-2 border-white" />
                                    <div className="w-8 h-8 rounded-full border-2 border-white bg-slate-100 flex items-center justify-center text-[8px] font-bold text-slate-500">+3</div>
                                </div>
                            </div>
                        )}
                        ...
                        {item.variant === 'typography-hero' && (
                            <div className="flex flex-col justify-between h-full relative p-4">
                                <div className="w-12 h-12 rounded-full border-2 border-slate-800 flex items-center justify-center mb-6">
                                    <FaQuoteRight className="text-xl text-slate-800" />
                                </div>
                                <div className="mb-8">
                                    <p className="text-[10px] uppercase tracking-widest text-slate-400 mb-2">What People Say</p>
                                ...
                                <div className="flex items-center gap-3 mt-auto">
                                    ...
                                    <span className="ml-auto font-handwriting text-slate-400 text-lg">dribbble</span>
                                </div>
                            </div>
                        )}
                        ...
                        {item.variant === 'centered-avatar' && (
                            <div className="text-center flex flex-col items-center pt-4">
                                ...
                                <span className="font-handwriting text-xl text-slate-400">Isabella ..</span>
                            </div>
                        )}
